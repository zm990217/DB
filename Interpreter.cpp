...
int Interpreter::interpret(string s) {
	int tmp = 0;
	string command = getCommand(s, tmp);
	...
	//select operation
		...
		if (command.empty()) {
		...
		else if (command == "where") {//with condition
			string attributeName = "";
			string value = "";
			int operate = Condition::OPERATOR_EQUAL;
			std::vector<Condition> conditionVector;
			command = getCommand(s, tmp);//get the attribute+		command	"id"	std::basic_string<char,std::char_traits<char>,std::allocator<char> >
			...
			while (1) {
				try {
					...
					if (command != "and")
						throw exception(ex);
					command = getCommand(s, tmp);//get the attribute
				}
				catch (exception& e) {
					cout << e.what() << endl;
					return 0;
			...
			while (1) {
				try {
					...
					if (command.empty())
						throw exception(ex);
					attributeName = command;
					command = getCommand(s, tmp);//get the operator
					if (command == "<=")
						operate = Condition::OPERATOR_LESS_EQUAL;
					else if (command == ">=")
						operate = Condition::OPERATOR_MORE_EQUAL;
...
string Interpreter::getCommand(string s, int &tmp) {
	...
	if (s[tmp] == '(' || s[tmp] == ',' || s[tmp] == ')') {//get a char
		tmp++;
		index2 = tmp;
