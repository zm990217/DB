...
public:
    ...
    BPlusTreeNode *Split(KeyType &key, offsetNum &val); // split the node by a key
...
private:
    void RecursivelySplit(BPlusTreeNode<KeyType> *node);
 ...
 * 
 * degree: the degree of the new node
 * isNewLeaf: flag that the node whether is a leaf node
 * 
 */
 ...
 * 
 * key: the key value we want to search
 * index: the position of the key we find, 
...
/**
 * split the node by a key
 * 
 * key: the key add to the parent node
 * 
 * return PBlusTreeNode: new node we get after the split
 * 
 */

template<typename KeyType>
BPlusTreeNode<KeyType> *BPlusTreeNode<KeyType>::Split(KeyType &key, offsetNum &val)
{
    if(keysNum == 0)
    {
    ...
    if(newNode == nullptr)
    {
        std::cout << "Problems in allocate memory in function split" << std::endl;
        exit(0);
    }

    //copy the right half hand of the node to the new node
    //consider that these vectors are already out of size
 ...
 * delete a key
 * 
 * index: the position of the key we want to delete
 * 
 * return bool: whether succeed delete the key
 * 
 */
 ...
 * index: the position of the key in which
 ...
 * val: the offsetnum of the record
 * 
 */
...
/**
 * split the parameter node. If its parent node is full
 * after then, split its parent. Do this process recursively
 * 
 * node: the first node to split
 * 
 */

template<typename KeyType>
void BPlusTree<KeyType>::RecursivelySplit(BPlusTreeNode<KeyType> *node)
{
    KeyType key;
    offsetNum val;
    BPlusTreeNode<KeyType> *newNode = node->Split(key, val);
    ...
    if(node->IsRoot() == true)
    {
        ...
        if(newRoot == nullptr)
        {
            std::cout << "Memery error recursive split" << std::endl;
            exit(3);
        }
 ...
 * 
 * node: the node we need to modify
 * 
...
bool BPlusTree<KeyType>::RecursivelyCombine(BPlusTreeNode<KeyType> *node)
{
    ...
    //get the location of the node in the parent node
    ...
    else if(node->isLeaf == true)
    {
        ...
        {
            //If the node is the last child of the father node, then choose the left sibling node to borrow
            brother = parent->childSet[index - 1];
            if(brother->keysNum > minNodeSize)
            {
                //borrow a value from a sibling node
                //the sibling node deletes the borrowed value, and the father node value changes accordingly
                //because is a leaf node, there is no need to consider the child node.
    ...
    else    //neither a root node nor a leaf node
    {
        if(index == parent->keysNum)
        {
            brother = parent->childSet[index -1];
 ...
 * 
 * node: the node you want to delete
 * 
 */
 ...
 * 
 * btmp: the node we need to read in
 * 
 */
