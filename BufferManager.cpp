...
BufferManager::BufferManager() :totalBlock(0), totalFile(0), fileHead(NULL) {
	for (int i = 0; i < MAX_FILE_NUM; i++) {
		...
		if (file_pool[i].fileName == NULL) {
			cout << "Fail to allocate memory in the file pool!" << endl;
			exit(1);
		}
	...
	for (int i = 0; i < MAX_BLOCK_NUM; i++) {
		...
		if (block_pool[i].address == NULL) {
			cout << "Fail to allocate memory in the block pool!" << endl;
			exit(1);
		}
		block_pool[i].fileName = new char[MAX_FILE_NAME];
		if (block_pool[i].fileName == NULL) {
			cout << "Fail to allocate memory to the block name!" << endl;
			exit(1);
		}
...
fileNode* BufferManager::getFile(const char * fileName, bool if_pin) {
	...
	if (fileHead != NULL) {
		for (ftmp = fileHead; ftmp != NULL; ftmp = ftmp->nextFile) {
			if (!strcmp(fileName, ftmp->fileName)) {
				//the file node is already in the list
				ftmp->pin = if_pin;
				return ftmp;
			}
		}
	}
	//the file node is not in the list
	if (totalFile == 0) {
		//no file is in the list
		ftmp = &file_pool[totalFile];
		totalFile++;
		fileHead = ftmp;
	}
	else if (totalFile < MAX_FILE_NUM) {
		//if there are empty file nodes in the pool
		ftmp = &file_pool[totalFile];
		//add this file node to the end of the list
		file_pool[totalFile - 1].nextFile = ftmp;
		ftmp->preFile = &file_pool[totalFile - 1];
		totalFile++;
	}
	else {
		//find a file node to replace and write back the block node belonging to it
		ftmp = fileHead;
		while (ftmp->pin) {
			if (ftmp->nextFile)ftmp = ftmp->nextFile;
			else {
				//no enough file nodes in the pool
				cout << "There are no enough file nodes in the pool!";
				exit(2);
			}
		}
	...
	if (strlen(fileName) + 1 > MAX_FILE_NAME) {
		cout << "The file name exceeds the maximum " << MAX_FILE_NAME << " !" << endl;
		exit(3);
	}
...
blockNode* BufferManager::getBlock(fileNode * file, blockNode *position, bool if_pin) {
	...
	else { //there are no empty block, so we need to replace one
	...
	if ((fileHandle = fopen(fileName, "ab+")) != NULL) {
		if (fseek(fileHandle, btmp->offsetNum*BLOCK_SIZE, 0) == 0) {
			...
			cout << "Fail to seek the file " << fileName << " in reading" << endl;
			exit(1);
		}
		fclose(fileHandle);
	}
	else {
		cout << "File to open the file " << fileName << " in reading" << endl;
		exit(1);
	}
	return btmp;
}
...
void BufferManager::writtenBackToDisk(const char* fileName, blockNode* block) {
	if (!block->dirty) //the block has not been modified, so it doesn't need to be written back
		return;
	else {
		...
		if ((fileHandle = fopen(fileName, "rb+")) != NULL) {
			if (fseek(fileHandle, block->offsetNum*BLOCK_SIZE, 0) == 0) {
				if (fwrite(block->address, block->usingSize + sizeof(size_t), 1, fileHandle) != 1) {
					cout << "Fail to write the file " << fileName << " in writtenBackToDisking";
					exit(1);
				}
			}
			else {
				cout << "Fail to seek the file " << fileName << " in writtenBackToDisking" << endl;
				exit(1);
			}
			fclose(fileHandle);
		}
		else {
			cout << "Fail to open the file " << fileName << " in writtenBackToDisking" << endl;
			exit(1);
		}
	}
}
