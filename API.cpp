...
void API::createIndex(string indexName, string tableName, string attributeName) {
	...
	if (rm->createIndex(indexName)) {
		...
		if (indexType == -2) {
			clock_t finish = clock();
			double duration = (double)(finish - start) / CLOCKS_PER_SEC;
			cout << "Error in creating index!(" << duration << " s)" << endl;
			return;
		}
...
void API::showRecord(string tableName, vector<string>* attributeNameVector, vector<Condition>* conditionVector) {
	if (cm->findTable(tableName) == TABLE_FILE) {
		...
		if (conditionVector != NULL) {
			for (Condition condition : *conditionVector) {
				...
				if (i == attributeVector.size()) {
					clock_t finish = clock();
					double duration = (double)(finish - start) / CLOCKS_PER_SEC;
					cout << "The attribute not exists in the table.(" << duration << " s)" << endl;
					return;
				}
			}
		}
		...
		if(num)
			tableBottomPrint(attributeNameVector);

		clock_t finish = clock();
		double duration = (double)(finish - start) / CLOCKS_PER_SEC;
		if(num)
			cout << "Query OK, total " << num << " records found. (" << duration << " s)" << endl;
		else
			cout << "No record was found that met the condition. (" << duration << " s)" << endl;
	}
	else {
		clock_t finish = clock();
		double duration = (double)(finish - start) / CLOCKS_PER_SEC;
...
void API::insertRecord(string tableName, vector<string>* recordContent) {//tablename valuevector(string)
	...
	for (int i = 0; i < attributeVector.size(); i++) {
		...
		if (indexName != "") {
			...
			if (blockoffest != -1) {
				...
				clock_t finish = clock();
				double duration = (double)(finish - start) / CLOCKS_PER_SEC;
				cout << "Insert failed because the index already exists.(" << duration << " s)" << endl;
				return;
			}
		}
	...
	if (conditionVector.size() > 0) {
		for (int i = 0; i < conditionVector.size(); i++) {
			...
			if (recordConflictNum > 0) {
				clock_t finish = clock();
				double duration = (double)(finish - start) / CLOCKS_PER_SEC;
				cout << "Insert failed because unique value exists.(" << duration << " s)" << endl;
				return;
			}
		}
	}
	...
	int num;
	num = cm->getRecordString(tableName, recordContent, recordString);
	if (num) {//if the record's type is correct
...
void API::tableAttributePrint(vector<string>* attributeNameVector) {
	...
	for (i = 0; i < (*attributeNameVector).size(); i++) {
		for (int j = 0; j < length + 10; j += 2)
			cout << "--";
	}
	cout << "-\n| ";//the first character of the second line
	for (i = 0; i < (*attributeNameVector).size(); i++) {
		cout << (*attributeNameVector)[i];
		for (int j = 0; j < length + 8 - (*attributeNameVector)[i].size(); j++)
			cout << " ";
		cout << "| ";//the vertical line in front of the attribute name
	}
	if (i != 0) {
		cout << "\n";
		for (i = 0; i < (*attributeNameVector).size(); i++) {
			for (int j = 0; j < length + 10; j += 2)
				cout << "--";//the lower part of the header
		}
		cout << "-";//the last character in the header
	}
	cout << "\n";
}

void API::tableBottomPrint(vector<string>* attributeNameVector) {
	int i, j;
	cout << "-";//the beginning of the last line
	for (i = 0; i < (*attributeNameVector).size(); i++) {
		for (j = 0; j < length + 10; j += 2)
			cout << "--";//the last line
	}
	cout << "\n";
}
